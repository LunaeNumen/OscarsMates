class UserPick < ApplicationRecord
  ...
  validates :user_id, uniqueness: { scope: [:category_id, :year], message: "can only pick one movie per category per year" }
  ...
  def movie_must_be_nominated_in_category
    ...
    unless Nomination.exists?(movie_id: movie_id, category_id: category_id, year: year)
